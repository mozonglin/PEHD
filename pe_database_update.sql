...
-- 为activities表添加审核状态相关字段
ALTER TABLE activities 
ADD COLUMN approval_status VARCHAR(20) NOT NULL DEFAULT 'PENDING' COMMENT '审核状态：PENDING-审核中，APPROVED-已通过，REJECTED-已拒绝，DRAFT-草稿',
ADD COLUMN reviewed_by VARCHAR(36) DEFAULT NULL COMMENT '审核人ID',
ADD COLUMN reviewed_at TIMESTAMP NULL DEFAULT NULL COMMENT '审核时间',
ADD COLUMN review_comment TEXT COMMENT '审核意见';

-- 为现有数据设置默认审核状态为已通过（保持现有活动可用）
UPDATE activities SET approval_status = 'APPROVED' WHERE approval_status = 'PENDING';

-- 添加审核状态相关索引
CREATE INDEX idx_approval_status ON activities(approval_status);
CREATE INDEX idx_organizer_approval ON activities(organizer_id, approval_status);

-- 验证更新结果
SELECT COUNT(*) as total_activities, approval_status 
FROM activities 
